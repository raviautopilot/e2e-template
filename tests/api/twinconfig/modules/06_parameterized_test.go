...
// TestAPI_Modules_Parameterized runs table-driven parameterized tests for Modules.
func TestAPI_Modules_Parameterized(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			name:           "Create Modules with empty body returns 400",
			description:    "Ensure validation rejects an empty payload",
			method:         "POST",
			path:           "/api/v1/config/modules",
		...
		{
			name:           "Get non-existent Modules ID returns 404",
		...
		{
			name:           "Delete non-existent Modules ID returns 404",
