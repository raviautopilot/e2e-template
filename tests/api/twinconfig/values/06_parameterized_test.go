...
// TestAPI_Values_Parameterized runs table-driven parameterized tests for Values.
func TestAPI_Values_Parameterized(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			name:           "Create Values with empty body returns 400",
			description:    "Ensure validation rejects an empty payload",
			method:         "POST",
			path:           "/api/v1/config/values",
		...
		{
			name:           "Get non-existent Values ID returns 404",
		...
		{
			name:           "Delete non-existent Values ID returns 404",
