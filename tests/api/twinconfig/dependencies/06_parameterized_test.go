...
// TestAPI_Dependencies_Parameterized runs table-driven parameterized tests for Dependencies.
func TestAPI_Dependencies_Parameterized(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			name:           "Create Dependencies with empty body returns 400",
			description:    "Ensure validation rejects an empty payload",
			method:         "POST",
			path:           "/api/v1/config/dependencies",
		...
		{
			name:           "Get non-existent Dependencies ID returns 404",
		...
		{
			name:           "Delete non-existent Dependencies ID returns 404",
