...
func TestAPI_HttpBin_05_Auth(t *testing.T) {
	tests.RunAPITestWithClients(t, "Bearer and Basic authentication endpoints with dual clients",
		...
		func(tc *tests.TestContext) {
			...
			actions.SendHttpRequest(tc, tc.Client, "GET", "/bearer", nil, nil, &bearerResp, bearerAuth)
			...
			actions.SendHttpRequest(tc, tc.Client2, "GET", "/basic-auth/user1/pass123", nil, nil, &basicResp, basicAuth)
